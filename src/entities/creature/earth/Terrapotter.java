...
public class Terrapotter extends Creature {
    public Terrapotter(int level) {
        super(level);
        element = Elements.EARTH;
        maxHealth = calculateLevelHealth();
        assignAttacks();
        name = "Terrapotter";
        sheetLeft = Media.sheetCreatureTerrapotterLeft;
        sheetRight = Media.sheetCreatureTerrapotterRight;
        cost = 450;
    }
    ...
    @Override public void assignAttacks() {
        if (curLevel < CREATURE_MAIN_SINGLE_UPGRADE_ONE) {
            mainSingleAttack = new EarthSingleAttackOne(0.67, levelDamageBonus);
        }
        else {
            mainSingleAttack = new EarthSingleAttackTwo(0.67, levelDamageBonus);
        }

        if (curLevel < CREATURE_MAIN_AREA_UPGRADE_ONE) {
            mainAreaAttack = new EarthAreaAttackOne(0.67, levelDamageBonus);
        }
        else {
            mainAreaAttack = new EarthAreaAttackTwo(0.67, levelDamageBonus);
        }
        ...
        if (curLevel < CREATURE_ALT_SINGLE_UNLOCK) {
            ...
            altSingleAttack = new FireSingleAttackOne(0.67, levelDamageBonus);
        }
        else {
            altSingleAttack = new FireSingleAttackTwo(0.67, levelDamageBonus);
        }
